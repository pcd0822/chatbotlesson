...
배포된 사이트의 **Site configuration → Environment variables**에서 다음 두 개를 추가하세요.

### `OPENAI_API_KEY`
[OpenAI 대시보드](https://platform.openai.com/api-keys)에서 발급받은 API 키 (`sk-...` 로 시작)

### `GOOGLE_SERVICE_ACCOUNT_JSON`
2단계에서 받은 JSON 파일을 **메모장 등으로 열어 전체 내용을 통째로 복사**해서 값으로 붙여넣으세요.

> 🌟 **팁**: JSON에 줄바꿈이 많으면 그대로 붙여넣어도 보통 동작하지만, 잘 안 되면 한 줄로 붙여 넣거나 base64로 인코딩한 값을 넣어도 됩니다. (코드가 두 형식 다 지원)

환경변수 등록 후, **반드시 다시 배포(Redeploy)** 하세요. (Deploys → Trigger deploy → Deploy site)
...
| `Google Drive 폴더를 읽지 못했습니다` | 폴더가 **서비스 계정 이메일에 공유**되어 있는지, 폴더 ID가 맞는지 |
